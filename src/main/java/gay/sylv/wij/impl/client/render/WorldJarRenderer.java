...
public class WorldJarRenderer implements BlockEntityRenderer<WorldJarBlockEntity> {
	...
	public void render(
		...
		if (jar.isStatesChanged()) {
			jar.setStatesChanged(false);
			buildJar(context, (ClientWorldJarBlockEntity) jar);
		}
		...
		jar.getChunkSections().forEach((pos, section) -> {
			...
			for (BlockPos blockPos : BlockPos.betweenClosed(origin, offset)) {
				...
				if (!fluidState.isEmpty()) {
					...
					context.getBlockRenderDispatcher().renderLiquid(blockPos, jar.getRenderChunkRegion(), bufferBuilder, state, fluidState);
				}
				...
				if (state.getRenderShape() == RenderShape.MODEL) {
					...
					context.getBlockRenderDispatcher().renderBatched(state, blockPos, jar.getRenderChunkRegion(), poseStack, bufferBuilder, true, randomSource);
					poseStack.popPose();
				}
			}
