...
public abstract class PlayerMixin extends LivingEntity implements PlayerWithReturn, PlayerWithEnteredJar {
	...
	public @NotNull ResourceKey<Level> worldinajar$getReturnDimension() {
		if (hasAttached(FabricAttachments.RETURN_JAR_LOCATION)) {
			return getAttachedOrThrow(FabricAttachments.RETURN_JAR_LOCATION).dimension();
		} else {
	...
	public @NotNull Vec3 worldinajar$getReturnPos() {
		if (hasAttached(FabricAttachments.PLAYER_RETURN_POS)) {
			return getAttachedOrThrow(FabricAttachments.PLAYER_RETURN_POS);
	...
	public void worldinajar$setReturnPos(@NotNull Vec3 pos) {
		setAttached(FabricAttachments.PLAYER_RETURN_POS, pos);
	}

	@Override
	public void worldinajar$setReturnLocation(Networking.@NotNull JarLocation returnLocation) {
		setAttached(FabricAttachments.RETURN_JAR_LOCATION, returnLocation);
	}

	@Override
	public void worldinajar$RemoveReturnLocation() {
		removeAttached(FabricAttachments.RETURN_JAR_LOCATION);
	}

	@Override
	public Optional<Networking.JarLocation> worldinajar$getJarLocation() {
		return Optional.ofNullable(getAttached(FabricAttachments.RETURN_JAR_LOCATION));
	}

	@Override
	public void worldinajar$setJarLocation(Networking.JarLocation location) {
		setAttached(FabricAttachments.RETURN_JAR_LOCATION, location);
	}
}
