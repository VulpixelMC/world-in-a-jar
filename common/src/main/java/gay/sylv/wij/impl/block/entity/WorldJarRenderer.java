...
public class WorldJarRenderer implements BlockEntityRenderer<WorldJarBlockEntity> {
	...
	public void render(
		...
		if (jar.level != null && jar.level.dimension().equals(Dimensions.JAR))
			return;
		poseStack.pushPose();
		// prevent z-fighting
		poseStack.scale(
			...
			for (BlockPos blockPos : BlockPos.betweenClosed(origin, offset)) {
				...
				if (!fluidState.isEmpty()) {
					...
					context.getBlockRenderDispatcher()
							.renderLiquid(blockPos, jar.renderChunkRegion, bufferBuilder, state, fluidState);
				}
				...
				if (state.getRenderShape() == RenderShape.MODEL) {
					...
					context.getBlockRenderDispatcher()
							.renderBatched(state, blockPos, jar.renderChunkRegion, poseStack, bufferBuilder, true,
									randomSource
							);
					poseStack.popPose();
				}
